   ...
   */
		resource[actionName] = function () {
			...
			switch (arguments.length) {
				...
				case 1:
					if (hasBody) {
						configs.data = arguments.length <= 0 ? undefined : Object.assign({}, action.params, arguments[0]);
					} else {
						params = arguments.length <= 0 ? undefined : Object.assign({}, action.params, arguments[0]);
					}
					break;

				case 0:
                    if (hasBody) {
                        configs.data = action.params;
                    } else {
			...
			var url = genUrlFromTemplate((configs.url || ''), extractParams);
			...
			configs.params = getRestParamsFromUrlTemplate(configs.url, extractParams);

			return (0, _fetchHttp2.default)(urlTemplate + url, method, configs).then(function (response) {
