...
function defaultRequestTransformer(data) {
	const isJson = (0, _objectUtil.isObject)(data) &&
	!(0, _objectUtil.isFile)(data) &&
	!(0, _objectUtil.isBlob)(data) &&
	!(0, _objectUtil.isFormData)(data)
	if(isJson){
        return toQueryString(data);//(0, _objectUtil.toJson)(data)
	}
	else{
		return data;
	}
...
function FetchHttp(url, method, configs) {
	...
	var serverRequest = function serverRequest(requestConfigs) {
        ...
        var bodyAfterTransform = null;
		if(requestConfigs.method.toUpperCase() == 'POST'){
            bodyAfterTransform = toQueryString(requestConfigs.data);
		}
		else{
            bodyAfterTransform = executeHttpTransformers(requestConfigs.data, getHeadersGetter(requestConfigs.headers), undefined, requestConfigs.requestTransformers);
		}
		var configsAfterTransform = Object.assign({ body: bodyAfterTransform }, requestConfigs);
		// build url
		if (configsAfterTransform.params) {
			var data = configsAfterTransform.params;
...
FetchHttp.defaultConfigs = {
	...
	headers: {
		'Content-Type': _httpConstants.APPLICATION_FORM + ';charset=utf-8',
